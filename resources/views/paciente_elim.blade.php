    ...
    <div class="container mx-auto  bg-white rounded-xl shadow mt-4 ">
        <div class="p-6 text-gray-900 ">
            <div class="container mx-auto  ">
                <h1 class="text-2xl font-bold mb-4">MASCOTAS ELIMINADAS</h1>
                <div class="flex flex-col ">
                    ...
                    <div class="flex  flex-col sm:flex-row justify-between mb-4 gap-3">
                                ...
                                <form action="{{ route('pacientes.forceDeleteAll') }}" method="POST"
                                    onsubmit="return confirm('¿Eliminar permanentemente todas las mascotas eliminadas?')"
                    ...
                    <table class="min-w-full text-sm text-gray-700">
                        <thead class="bg-gray-100 text-gray-700 uppercase text-xs">
                                <tr >
                                    ...
                                    <th scope="col" class="border  px-4 py-2 text-black">Especie</th>
                                    <th scope="col" class="border  px-4 py-2 text-black">Raza</th>
                                    <th scope="col" class="border  px-4 py-2 text-black">Edad</th>
                            ...
                            <tbody class="bg-white text-center text-black">
                                @forelse ($deletedPacientes as $paciente)
                                    <tr>
                                        <td class="border  px-4 py-2 text-start">{{ $paciente->id }}</td>
                                        <td class="border  px-4 py-2 text-start">{{ $paciente->nombre }}</td>
                                        <td class="border  px-4 py-2 text-start">{{ $paciente->especie }}</td>  
                                        <td class="border  px-4 py-2 text-start">{{ $paciente->raza }}</td>
                                        <td class="border  px-4 py-2 text-start">{{ $paciente->edad }}</td>

                                        <td class="border  px-4 py-2 text-gray-500">
                                            {{ $paciente->deleted_at->format('d/m/Y H:i') }}
                                        </td>
                                        <td class=" border  py-4 text-center space-x-2">
                                            <form action="{{ route('pacientes.restore', $paciente->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 transition">
                                                    Restaurar
                                                </button>
                                            </form>
                                            <form action="{{ route('pacientes.forceDelete', $paciente->id) }}" method="POST" class="inline"
                                                  onsubmit="return confirm('¿Estás seguro de que deseas eliminar permanentemente este paciente? Esta acción no se puede deshacer.')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                       <td colspan="8" class="border  px-4 py-2 text-gray-500"><ion-icon name="alert-circle-outline" class="text-gray-400 text-2xl"></ion-icon>No hay mascotas eliminadas.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
