<x-app-layout>
        ...
        <div class="container mx-auto  ">
            <h1 class="text-2xl font-bold mb-4">SERVICIOS ELIMINADOS</h1>
        ...
        <div class="py-6">
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                 ...
                 <div class="flex  flex-col sm:flex-row justify-between mb-4 gap-3">
                                ...
                                <form action="{{ route('servicios.forceDeleteAll') }}" method="POST"
                                    onsubmit="return confirm('¿Eliminar permanentemente todos los servicios eliminados?')"
                ...
                <div class="bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm text-gray-700">
                            ...
                            <tbody>
                                @forelse ($deletedServicios as $servicio)
                                    <tr class="text-center border-t hover:bg-gray-50 transition-all">
                                        ...
                                        <td class="px-4 py-2">{{ Str::limit($servicio->descripcion, 50) }}</td>
                                        <td class="px-4 py-2">{{ number_format($servicio->precio, 2) }}</td>
                                        <td class="px-4 py-2 text-gray-500">
                                            {{ $servicio->deleted_at->format('d/m/Y H:i') }}
                                        </td>   
                                        <td class="px-4 py-2">
                                            <div class="flex justify-center space-x-2">
                                                <!-- Restaurar -->
                                                <form action="{{ route('servicios.restore', $servicio->id) }}" method="POST">
                                                    @csrf
                                                ...
                                                <form action="{{ route('servicios.forceDelete', $servicio->id) }}" method="POST"
                                                      onsubmit="return confirm('¿Eliminar permanentemente este servicio?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-4 py-2 text-center text-gray-500">No hay servicios eliminados.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    @if ($deletedServicios->hasPages())
                        <div class="px-4 py-3 bg-gray-50 border-t">
                            {{ $deletedServicios->links() }}
                        </div>
                    @endif
                </div>
            </div>
