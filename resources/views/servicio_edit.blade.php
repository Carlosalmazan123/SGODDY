<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto container bg-white p-6 rounded-lg shadow-md">
        ...
        <form action="{{ route('servicios.update', $servicio->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="duracion" class="block text-gray-700 font-medium">Duración</label>
                <input type="time" name="duracion" id="duracion" class="w-full p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('duracion', $servicio->duracion) }}" required>
            </div>
